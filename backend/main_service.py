...
class MainService:
    ...
    def predict(self) -> str:
        ...
        parced_cards = [Card(name=card.card_name, description=card.regular_desc if random.choice([False, True]) else card.flipped_desc) for card in cards]

        return self._AIService.make_prediction(predict_type=self.predict_type, prompt=self.prompt, cards=parced_cards)
